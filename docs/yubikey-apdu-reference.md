...
00 87 <algo> <slot> <Lc>
  7C <n>
    82 00               (empty response placeholder — required)
...
**1. AES-256 P1 byte was wrong — FIXED**

`authenticate_management_key` previously used `0x0E` for AES-256; correct value is `0x0C`
per yubico-piv-tool (`YKPIV_ALGO_AES256 = 0x0c`) and ykman (`AES256 = 0x0C`).
Fixed in `hardware.rs`.

**2. ECDH missing `82 00` response placeholder — FIXED**

Our ECDH payload previously sent `7C <n> 85 <n> <peer_point>` without the `82 00`
response placeholder. Both yubico-piv-tool and ykman include it:
```
7C <n>  82 00  85 <n> <peer_point>
```
Fixed in `hardware.rs`. `general_authenticate_sign` already had it correctly.

### Known gaps (not bugs)

**3. Algorithm auto-detection not implemented**

yubico-piv-tool calls GET_METADATA (INS=F7) before every management key auth to determine
whether the slot holds 3DES or AES. Our code decodes purely from key byte length. Works
for all common key lengths (16, 24 for 3DES, 32 bytes) but cannot distinguish AES-192
from 3DES (both 24 bytes). AES-192 management keys are extremely rare.

**4. Le byte after GET DATA APDU (benign)**

Our GET DATA APDU includes a trailing `0x00` Le byte. yubico-piv-tool adds Le only for
T=1 protocol; ykman omits it. The card ignores a spurious `Le=00`, so this is benign.

**5. GET_METADATA (INS=F7) not implemented**

Not needed for current yb use cases. The emulated backend returns a minimal stub for the
default-credential check path.
