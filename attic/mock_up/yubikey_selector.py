...
"""
YubiKey Selection Program with Interactive Menu and LED Feedback.
...
def main() -> int:
    ...
    try:
        ...
        print("Error: ykman library not available")
        return 1

    # Retrieve all connected YubiKeys
    print("Detecting YubiKeys...")
    devices_raw = list_all_devices()

    if not devices_raw:
        print("No YubiKeys found.")
        return 1

    # Extract (serial, version) tuples
    devices = [(info.serial, str(info.version)) for _, info in devices_raw]

    if len(devices) == 1:
        # Only one device - auto-select
        serial, version = devices[0]
        print( "\nOnly one YubiKey detected:")
        print(f"  YubiKey {serial} (v{version})\n")
        print(f"Selected YubiKey: {serial}")
        return 0

    # Multiple devices - show interactive selector
    selector = YubiKeySelector(devices)
    selected_serial = selector.run()

    if selected_serial is None:
        print("\nSelection cancelled.")
        return 1

    # Display the folded result
    print("\n" + "─" * 40)
    print(f"Selected YubiKey: {selected_serial}")
    print("─" * 40 + "\n")
